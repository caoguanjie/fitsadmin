<template>
    <div class="fits-table">
        ...
        <table-group-pagination :total="tablePagination.total" v-model:page="tablePagination.pageIndex"
            v-model:limit="tablePagination.pageSize" @pagination="paginationChange" />
    </div>
</template>

<script lang="ts" setup name="FitsTableGroup">
import { nextTick, onActivated, onDeactivated, onMounted, onUnmounted, reactive, ref, toRefs, watch } from "vue";
import FitsCommonTable from "./components/CommonGroupTable.vue"
import FitsGroupConditions from "./components/GroupConditions.vue"
import FitsGroupTable from "./components/ConditionsGroupTable.vue"
import { FitsTableGroupProps, HttpType, ICustomColumnsConfig, IFistTableData, IFitsGroupCondition, IFitsLoadMoreParams, IFitsTableGroup } from "./type";
import { useColumns } from "./hooks/useColumns";
import { useCancelToken } from "./hooks/useCancelToken";
import _ from 'lodash';
import { ElMessage } from "element-plus";
import TableGroupPagination from "./components/TableGroupPagination.vue"

const emit = defineEmits(["expandChange"])
const props = defineProps<{ config: FitsTableGroupProps }>()
...
onActivated(() => {
    // 更新表格配置
    isFirstRender.value === false && updateColumns(useColumns(config.value.headers, String(route.name)))
})

onMounted(() => {
    // 更新表格配置
    updateColumns(useColumns(config.value.headers, String(route.name)))
...
async function fetchTableData(groupParams?: IFitsLoadMoreParams) {
    ...
    await updateColumns(useColumns(config.value.headers, String(route.name)))
}
