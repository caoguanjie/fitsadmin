<template>
  ...
  <div class="node-panel">
    <div class="node-item" v-for="item in props.nodeList" :key="item.text" @mousedown="dragNode(item)">
      <div class="node-item-icon" :class="item.class">
        <div v-if="item.type === 'user' || item.type === 'time'" class="shape" />
        <span v-if="
          item.type === '阶段' || item.type === '入院' || item.type === '出院'
        " class="node-label">{{ item.text }}</span>
      </div>
      <span v-if="
        item.type === 'user' ||
        item.type === 'push' ||
        ...
        item.type === 'task' ||
        item.type === 'end'
      " class="node-label2">{{ item.text }}</span>
    </div>
  </div>
</template>
...
.node-panel {
  ...
  .node-item {
    ...
    .node-start1 {
      background: rgb(121, 201, 0);
      border-radius: 5px;
      width: 40px;
    }

    .node-rect1 {
      background: rgb(84, 170, 201);
      border-radius: 5px;
      width: 40px;
    }

    .node-end1 {
      background: rgb(179, 127, 235);
      border-radius: 5px;
      width: 40px;
    }
