                        ...
                        <vue-draggable-next v-model="customTableCols" handle=".sortColsHandle">
                            <transition-group>
                                <div v-for="col in customTableCols" :key="((col.field || col.type) as string)"
                                    ...
                                    <img src="/src/assets/group-table/sort.png" width="14" height="14" alt=""
                                        class="sortColsHandle">
                                </div>
                            </transition-group>
                        </vue-draggable-next>
                    </el-scrollbar>
...
watch(() => props.headers, val => {
    if (val && val.length > 0) {
        customTableCols.value = JSON.parse(JSON.stringify(useColumns(val)))
        lastColsConfig.value = JSON.parse(JSON.stringify(useColumns(val)))
    }
